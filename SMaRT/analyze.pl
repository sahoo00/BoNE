...
if (scalar(@ARGV) < 1) {
    print "Usage: analyze.pl cmd <args> \n";
    print "cmd args:\n";
    print "   text <?.annot.csv>\n";
    exit(1);
}
...
sub analyzeNetwork {
  ...
  if (defined $params[0] && $params[0] eq "state") {
    ...
    my $start = $h->getStart();
    my $end = $h->getEnd();
    my $mtype = $h->getSurvivalName("c Type");
    my $m0 = [ grep { $mtype->[$_] eq "M0" } $start .. $end ];
    my $m1 = [ grep { $mtype->[$_] eq "M1" } $start .. $end ];
    my $m2 = [ grep { $mtype->[$_] eq "M2" } $start .. $end ];
